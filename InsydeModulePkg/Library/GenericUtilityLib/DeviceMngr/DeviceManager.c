...
STATIC EFI_HII_DATABASE_PROTOCOL     *gDMHiiDatabase;
STATIC EFI_FORM_BROWSER2_PROTOCOL    *gDMFormBrowser2;
STATIC UINTN                         gDMCallbackKey;

STATIC DEVICE_MANAGER_CALLBACK_DATA  gDeviceManagerPrivate = {
...
//
STATIC EFI_STRING  mSelectedMacAddrString;

//
// Which form Id need to be show.
//
STATIC EFI_FORM_ID      mNextShowFormId = DEVICE_MANAGER_FORM_ID;

//
// The Mac Address show in the NETWORK_DEVICE_LIST_FORM_ID
//
STATIC MAC_ADDRESS_NODE_LIST  mMacDeviceList;


STATIC HII_VENDOR_DEVICE_PATH  mDeviceManagerHiiVendorDevicePath = {
  {
    {
...
STATIC HII_VENDOR_DEVICE_PATH  mDriverHealthHiiVendorDevicePath = {
  {
    {
      HARDWARE_DEVICE_PATH,
        HW_VENDOR_DP,
  ...
  )
{
  ...
  Status = gBS->LocateProtocol (&gEfiHiiDatabaseProtocolGuid, NULL, (VOID **) &gDMHiiDatabase);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = gBS->LocateProtocol (&gEfiFormBrowser2ProtocolGuid, NULL, (VOID **) &gDMFormBrowser2);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  *FormSetHelp  = 0;
  ...
  BufferSize = 0;
  HiiPackageList = NULL;
  Status = gDMHiiDatabase->ExportPackageLists (gDMHiiDatabase, Handle, &BufferSize, HiiPackageList);
  //
  ...
  )
{
  ...
  *ItemCount = 0;
  ...
  Status = gDMHiiDatabase->GetPackageListHandle (gDMHiiDatabase, Handle, &DriverHandle);
  if (EFI_ERROR (Status)) {
    return FALSE;
  }
  //
  ...
  )
{
  ...
  if (NumHandles != 0) {
    ...
    if (!PlaformHealthStatusCheck ()) {
      ...
      //
      HiiSetString (HiiHandle, STRING_TOKEN (STR_DRIVER_HEALTH_ALL_HEALTHY), GenericUtilityLibGetStringById (STRING_TOKEN (STR_DRIVER_NOT_HEALTH)), NULL);
    } else {
      //
      // For the string of STR_DRIVER_HEALTH_ALL_HEALTHY previously has been updated and we need to update it while re-entry.
      //
      HiiSetString (HiiHandle, STRING_TOKEN (STR_DRIVER_HEALTH_ALL_HEALTHY), GenericUtilityLibGetStringById (STRING_TOKEN (STR_DRIVER_HEALTH_ALL_HEALTHY)), NULL);
    }
  }
  ...
  HiiCreateOneOfOptionOpCode (
    ...
    gDeviceManagerPrivate.VideoBios ? 0 : EFI_IFR_OPTION_DEFAULT,
  ...
  HiiCreateOneOfOptionOpCode (
    ...
    gDeviceManagerPrivate.VideoBios ? EFI_IFR_OPTION_DEFAULT : 0,
    EFI_IFR_NUMERIC_SIZE_1,
    1
    );
  ...
  // a target to display
  //
  if ((gDMCallbackKey >= DEVICE_KEY_OFFSET)) {
    ActionRequest = EFI_BROWSER_ACTION_REQUEST_NONE;
    Status = gDMFormBrowser2->SendForm (
                                gDMFormBrowser2,
  ...
  //
  if (gDMCallbackKey == QUESTION_NETWORK_DEVICE_ID) {
  ...
  //
  if ((gDMCallbackKey < MAX_KEY_SECTION_LEN + NETWORK_DEVICE_LIST_KEY_OFFSET ) && (gDMCallbackKey >= NETWORK_DEVICE_LIST_KEY_OFFSET)) {
  ...
  )
{
  ...
  while (!IsNull (&DriverHealthList, Link)) {
    ...
    if ((DriverHealthInfo->MessageList != NULL) && (DriverHealthInfo->MessageList->StringId != 0)) {
      ...
      case EfiDriverHealthStatusRepairRequired:
        TmpString = GenericUtilityLibGetStringById (STRING_TOKEN (STR_REPAIR_REQUIRED));
        break;
      case EfiDriverHealthStatusConfigurationRequired:
        TmpString = GenericUtilityLibGetStringById (STRING_TOKEN (STR_CONFIGURATION_REQUIRED));
        break;
      case EfiDriverHealthStatusFailed:
        TmpString = GenericUtilityLibGetStringById (STRING_TOKEN (STR_OPERATION_FAILED));
        break;
      case EfiDriverHealthStatusReconnectRequired:
        TmpString = GenericUtilityLibGetStringById (STRING_TOKEN (STR_RECONNECT_REQUIRED));
        break;
      case EfiDriverHealthStatusRebootRequired:
        TmpString = GenericUtilityLibGetStringById (STRING_TOKEN (STR_REBOOT_REQUIRED));
        break;
      default:
        TmpString = GenericUtilityLibGetStringById (STRING_TOKEN (STR_DRIVER_HEALTH_HEALTHY));
        break;
      }
    }
    ...
    TokenHelp = HiiSetString (HiiHandle, 0, GenericUtilityLibGetStringById( STRING_TOKEN (STR_DH_REPAIR_SINGLE_HELP)), NULL);
  ...
  TokenHelp = HiiSetString (HiiHandle, 0, GenericUtilityLibGetStringById( STRING_TOKEN (STR_DH_REPAIR_ALL_HELP)), NULL);

  if (PlaformHealthStatusCheck ()) {
    //
    // No action need to do for the platform
    //
    Token = HiiSetString (HiiHandle, 0, GenericUtilityLibGetStringById( STRING_TOKEN (STR_DRIVER_HEALTH_ALL_HEALTHY)), NULL);
    HiiCreateActionOpCode (
    ...
    //
    Token = HiiSetString (HiiHandle, 0, GenericUtilityLibGetStringById( STRING_TOKEN (STR_DH_REPAIR_ALL_TITLE)), NULL);
    HiiCreateActionOpCode (
      StartOpCodeHandleRepair,
  ...
  //
  if (gDMCallbackKey >= DRIVER_HEALTH_KEY_OFFSET && gDMCallbackKey != DRIVER_HEALTH_REPAIR_ALL_KEY) {
    ActionRequest = EFI_BROWSER_ACTION_REQUEST_NONE;
