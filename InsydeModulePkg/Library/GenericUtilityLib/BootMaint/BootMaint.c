...
STATIC EFI_HII_CONFIG_ROUTING_PROTOCOL    *gBMHiiConfigRouting;
  ...
  )
{
  ...
  *Progress = Request;
  ...
  Status = gBMHiiConfigRouting->BlockToConfig (
                                  gBMHiiConfigRouting,
                                  ConfigRequest,
  ...
  )
{
  ...
  if (!FeaturePcdGet (PcdH2OFormBrowserSupported) && Action == EFI_BROWSER_ACTION_CHANGING) {
    ...
    //
    if (QuestionId == FORM_RESET ||
        QuestionId == KEY_VALUE_SAVE_AND_EXIT ||
        QuestionId == KEY_VALUE_NO_SAVE_AND_EXIT
        ) {
  ...
  )
{
  ...
  BmmCallbackInfo->BmmConfigAccess.RouteConfig   = BMFakeRouteConfig;
  BmmCallbackInfo->BmmConfigAccess.Callback      = BootMaintCallback;
  BmmCallbackInfo->BmmPreviousPageId             = FORM_MAIN_ID;
  BmmCallbackInfo->BmmCurrentPageId              = FORM_MAIN_ID;
  BmmCallbackInfo->FeConfigAccess.ExtractConfig  = BMFakeExtractConfig;
  BmmCallbackInfo->FeConfigAccess.RouteConfig    = BMFakeRouteConfig;
  ...
  Status = gBS->LocateProtocol (&gEfiHiiConfigRoutingProtocolGuid, NULL, (VOID **) &gBMHiiConfigRouting);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  CallbackData->FeCurrentState = FileExplorerStateBootFromFile;
