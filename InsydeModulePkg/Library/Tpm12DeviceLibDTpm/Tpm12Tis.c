  ...
  )
{
  ...
  RegRead = MmioRead32 ((UINTN)&TisReg->Access);
  if ((BOOLEAN)((UINT8)RegRead == (UINT8)-1)) {
    return FALSE;
  }
  
  //
  // TPM 2.0 CRB access is detected if TPM_ACCESS value is more than 1 byte.
  //
  return (BOOLEAN)((RegRead & 0xFFFFFF00) == 0xFFFFFF00);
}
