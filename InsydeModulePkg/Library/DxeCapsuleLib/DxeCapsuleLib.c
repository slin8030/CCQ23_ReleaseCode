...
STATIC FMP_IMAGE_INFO                        *mFmpImageInfo = NULL;
  ...
  )
{
  ...
  if (mFmpImageInfo != NULL) {
    for (FmpImageInfo = mFmpImageInfo; FmpImageInfo->Fmp != NULL; FmpImageInfo++) {
      gRT->ConvertPointer (0x0, (VOID **) &FmpImageInfo->Fmp);
      gRT->ConvertPointer (0x0, (VOID **) &FmpImageInfo->ImageInfo);
    }
    gRT->ConvertPointer (0x0, (VOID **) &mFmpImageInfo);
  }
}

/**
  Internal function to free mFmpImageInfo resource.
**/
STATIC
VOID
FreeFmpImageInfo (
  ...
  )
{
  ...
  for (FmpImageInfo = mFmpImageInfo; FmpImageInfo->Fmp != NULL; FmpImageInfo++) {
    FreePool (FmpImageInfo->ImageInfo);
  }
  FreePool (mFmpImageInfo);
  mFmpImageInfo = NULL;
}
...
/**
  ...
  @retval Other                  return value from Fmp->GetImageInfo () is failure.
**/
STATIC
EFI_STATUS
SetFmpImageInfo (
  ...
  )
{
  ...
  if (mVirtualAddressChangeEvent == NULL) {
    mFmpImageInfo = AllocateZeroPool ((NumberOfHandles + 1) * sizeof (FMP_IMAGE_INFO));
  } else {
    mFmpImageInfo = AllocateRuntimeZeroPool ((NumberOfHandles + 1) * sizeof (FMP_IMAGE_INFO));
  }
  if (mFmpImageInfo == NULL) {
  ...
  for(Index = 0, FmpImageInfo = mFmpImageInfo; Index < NumberOfHandles; Index++) {
    ...
    if (Status == EFI_SUCCESS) {
      Status = SetFmpImageInfo (Fmp, FmpImageInfo);
      if (Status == EFI_SUCCESS) {
        FmpImageInfo++;
      }
    }
  ...
  )
{
  ...
  if (CapsuleHeader == NULL || mFmpImageInfo == NULL) {
