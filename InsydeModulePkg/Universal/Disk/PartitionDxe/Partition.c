  ...
  for (Index = 0; Index < NumberOfChildren; Index++) {
    ...
    if (BlockIo2 != NULL) {
      ...
      Status = gBS->UninstallMultipleProtocolInterfaces (
                       ...
                       &gEfiBlockIo2ProtocolGuid,
                       &Private->BlockIo2,
                       CompareGuid (&Private->PartitionTypeGuid, &gEfiPartTypeUnusedGuid) ? NULL : &Private->PartitionTypeGuid,
                       NULL,
                       NULL
                       );
    } else {
...
/**
  ...
  @param[in]  PartitionTypeGuid partition type GUID defines the purpose and type of this Partition.
  ...
  )
{
  ...
  if (PartitionTypeGuid != NULL) {
    CopyMem (&Private->PartitionTypeGuid, PartitionTypeGuid, sizeof (EFI_GUID));
  }
  //
