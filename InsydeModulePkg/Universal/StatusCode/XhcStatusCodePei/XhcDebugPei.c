  ...
  )
{
  ...
  BarAddress &= ~0x0f;
  if (BarAddress != 0) {
    if (BarAddress != DebugIoConfig.XhciBar) {
  ...
  PciWrite (EfiWidthUint32, PciAddress + 0x10, &DebugIoConfig.XhciBar);
  return DebugIoConfig.XhciBar;
}
  ...
  )
{
  ...
  // XHCI detection
  //
  HcPort = DebugIoConfig.Config.UsbHcPort;
  Bridge = 0;
  Device = 0;
  Found  = FALSE;
  ...
  )
{
  ...
  if (!RuntimeState) {
    ...
    *(UINT32*)(UINTN)(Bar + (*(UINT32*)(UINTN)Bar & 0xff)) = 0x02;
    ...
    if (DebugContextAddr == NULL || *DebugContextAddr == 0) {
      Pointer = 0;
      Status = PeiServicesAllocatePages (EfiACPIMemoryNVS, 1, &Pointer);
  ...
  PostCode = 0xd0;
  Index = 0;
  Timeout = DebugIoConfig.Config.BypassTimeout * 2;
  while (TRUE) {
    ReadXhcDebugReg (DbgBar, EfiWidthUint32, XHCI_DCCTRL, &Data);
    if (Data & B_DCCTRL_DCR) break;
    MicroSecondDelay (1000);
    if (!Index) {
      IoWrite8((UINT16)DebugIoConfig.Config.DebugPort, PostCode);
  ...
  )
{
  ...
  for (StringPtr = OutputString; StringLength >= BufferLen; StringLength -= BufferLen, StringPtr += BufferLen) {
    ...
    DebugUsbWrite (Private, BufferLen, TempBuffer);
  }

  if (StringLength > 0) {
    CopyMem (TempBuffer, StringPtr, StringLength);
    DebugUsbWrite (Private, StringLength, TempBuffer);
  }
