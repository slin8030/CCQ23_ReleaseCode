...
STATIC
UINT32
Tcg2PpSubmitRequestToPreOSFunction (
...
UINT32
EFIAPI
Tcg2PpReturnOperationResponseToOsFunction (
  ...
  )
{
  ...
  if (ActivePcrBanks == mTcgDxeData.BsCap.ActivePcrBanks) {
    ...
    //
    ReturnCode = Tcg2PpSubmitRequestToPreOSFunction (TCG2_PHYSICAL_PRESENCE_NO_ACTION, 0);
  } else {
    ReturnCode = Tcg2PpSubmitRequestToPreOSFunction (TCG2_PHYSICAL_PRESENCE_SET_PCR_BANKS, ActivePcrBanks);
  }
  ...
  )
{
  ...
  ReturnCode = Tcg2PpReturnOperationResponseToOsFunction (OperationPresent, Response);
  ...
  )
{
  ...
  CopyMem (AuthPolicy.buffer, PcdGetPtr (PcdTpm2PhPolicySha256), AuthPolicy.size);
...
/**
  ...
  Set specific platformPolicy and randomize the platformAuth.
  ...
  )
{
  ...
  //
  // Set platformPolicy if PcdTpm2PhPolicySha256 has values
  //
  if (!IsZeroBuffer (PcdGetPtr (PcdTpm2PhPolicySha256), SHA256_DIGEST_SIZE)) {
    Status = SetPlatformPolicy ();
    if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if ((mTcgDxeData.BsCap.ProtocolVersion.Major > 0x01) || 
      ((mTcgDxeData.BsCap.ProtocolVersion.Major == 0x01) && ((mTcgDxeData.BsCap.ProtocolVersion.Minor > 0x00)))) {
    //
    // ASSERT if SHA256 bank is not supported
    //
    ASSERT ((mTcgDxeData.BsCap.ActivePcrBanks & EFI_TCG2_BOOT_HASH_ALG_SHA256) != 0);
  }
