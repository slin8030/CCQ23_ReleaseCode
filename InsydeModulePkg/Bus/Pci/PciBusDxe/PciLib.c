  ...
  )
{
  ...
  if (IS_PCI_BRIDGE(Pci)) {
    ...
    if ((CapIdPcie == EFI_SUCCESS) && (CapIdPcix == EFI_SUCCESS)) {
      //
      // Pcie to Pcix/Pci bridge. The value of secondary latnecy timer depends on child device type.
    ...
    if ((CapIdPcie == EFI_SUCCESS) && (CapIdPcix != EFI_SUCCESS)) {
      //
      // Pci-e header type 1. The latency timer must be hardwied to 0.
      //
      return EFI_SUCCESS;
    }
    ...
    if ((CapIdPcie != EFI_SUCCESS) && (CapIdPcix != EFI_SUCCESS)) {
      ...
      // Suggests to progrqm 0x20 for Pci device.
      //
      Data = 0x20;
    ...
    if ((CapIdPcie == EFI_SUCCESS) && (CapIdPcix != EFI_SUCCESS)) {
      //
      // Pci-e header type 0. The latency timer must be hardwied to 0.
      //
      return EFI_SUCCESS;
    }
  ...
  )
{
  ...
  if ((BridgeResource != NULL) && (BridgeResource->Length != 0)) {
    DEBUG ((
      EFI_D_INFO, "Type = %s; Base = 0x%lx;\tLength = 0x%lx;\tAlignment = 0x%x\n",
  ...
  )
{
  ...
  for (Link = BridgeResource->ChildList.ForwardLink; Link != &BridgeResource->ChildList; Link = Link->ForwardLink) {
    Resource = RESOURCE_NODE_FROM_LINK (Link);
  ...
  )
{
  ...
  for (Link = Bridge->ChildList.ForwardLink; Link != &Bridge->ChildList; Link = Link->ForwardLink) {
