...
{
  SIO_PRIVATE_DATA                *SioPrivateData;

  SioPrivateData = SIO_PRIVATE_FROM_THIS (This);

  //
  // Retrive uncommitted data from Browser
  //
  if (!HiiGetBrowserData (&mSioFormSetGuid, mSioVariableName, sizeof (SIO_CONFIGURATION), (UINT8 *) &SioPrivateData->Configuration)) {
    return EFI_NOT_FOUND;
  }
  ...
  FreePool (&SioPrivateData->Configuration);
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    while (!((PcdPointer->TypeH == NONE_ID) && (PcdPointer->TypeL == NONE_ID))) {    
      if ((PcdPointer->Device == COM) || (PcdPointer->Device == LPT) || (PcdPointer->Device == WDT) || (PcdPointer->Device == CIR) ||
        (PcdPointer->Device == IR)) {        
