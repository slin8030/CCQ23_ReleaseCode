...
extern VOID ProgramLpt (UINT8*, SIO_DEVICE_LIST_TABLE*);
extern VOID ProgramGpio (UINT8*, SIO_DEVICE_LIST_TABLE*);
extern VOID ProgramCom (UINT8*, SIO_DEVICE_LIST_TABLE*);
  ...
  )
{
  ...
  mDmiTablePtr = (UINT8*)PcdGetPtr (PcdSioNct5104dDmi);
  ...
  //
  if (FeaturePcdGet (PcdSioNct5104dSetup)) {
    Status = SioScu ();
  ...
  while (!((PcdPointer->TypeH == NONE_ID) && (PcdPointer->TypeL == NONE_ID))) {
    ...
    if (Different != 0) {
      ...
      if (PcdDmiPointer != NULL) {
	    	switch(Different){
			  case COM:
          ProgramCom(PcdDmiPointer, PcdPointer);
			    break;
			  case LPT:
          ProgramLpt(PcdDmiPointer, PcdPointer);
			    break;
			  case SGPIO:
          ProgramGpio(PcdDmiPointer, PcdPointer);
			    break;
		    }
      }
    }
  ...
  while (Status == EFI_SUCCESS) {
    ...
    if (Status == EFI_SUCCESS) {
      ...
      case (EFI_SIGNATURE_64 ('S', 'I', 'O', 'A', 'C', 'P', 'I', 0)):
        ...
        if(FeaturePcdGet(PcdSioNct5104dUpdateAsl)) {
