...
*/
{
  ...
  ECValue = CompalECACPINVSReadByte (EC_NAME_CPU_POWER_MANAGEMENT_OFFSET) & ~EC_NAME_CPU_MAX_PSTATE_MASK_BIT;
  ECValue |= MaxPstate << EC_NAME_CPU_MAX_PSTATE_SHIFT;
  CompalECACPINVSWriteByte (EC_NAME_CPU_POWER_MANAGEMENT_OFFSET, ECValue);
}
...
//  EFI_CPUID_REGISTER  Cpuid01;
  ...
  ECValue = CompalECACPINVSReadByte (EC_NAME_CPU_POWER_MANAGEMENT_OFFSET) & ~EC_NAME_CTDP_FLAG_MASK_BIT;
...
/*
Routine Description: \n
  - Detect CPU TurboBoost Supported or not then Set turbo mode Flag into EC Name Space
    Turbo_Mode(0:support,1:not support)  ; AFh.1
Arguments:
  - None
...
*/
{
  ...
  // Get and Clear cTDP Flag of EC Name Space Offset 0xF4AF, BIT1
  ECValue = CompalECACPINVSReadByte (EC_NAME_CPU_POWER_MANAGEMENT_OFFSET) & ~EC_NAME_TURBO_FLAG_MASK_BIT;
  ...
  if ((RegEax & TURBO_MODE_SUPPORTED) != TURBO_MODE_SUPPORTED)
  {
    ECValue |= EC_NAME_TURBO_FLAG_MASK_BIT; // Set BIT1 is not support.
  }

  // Pass Turbo Flag to EC Name Space
  CompalECACPINVSWriteByte (EC_NAME_CPU_POWER_MANAGEMENT_OFFSET, ECValue);
}
