...
//[PRJ]+ >>>> implement Touch Button function
  ...
  {
    Notify(\_SB.GPO0.HIDD, 0xC4) // Notify Virtual GPIO driver that Volume UP button is pressed.
    Notify(\_SB.GPO0.HIDD, 0xC5) // Notify Virtual GPIO driver that Volume UP button is released.
    sleep(300) //Add delay for debounce
  }
    
  Method(_L2B, 0x0, NotSerialized)
  {
    Notify(\_SB.GPO0.HIDD, 0xC6) // Notify Virtual GPIO driver that Volue Down button is pressed.
    Notify(\_SB.GPO0.HIDD, 0xC7) // Notify Virtual GPIO driver that Volue Down button is released.
    sleep(300) //Add delay for debounce
  }  
    
  Method(_L2C, 0x0, NotSerialized)
  {
    Notify(\_SB.PCI0.GFX0.DD1F, 0x86)  //Brightness up
	sleep(300) //Add delay for debounce
  }  
    
  Method(_L2D, 0x0, NotSerialized)
  {
    Notify(\_SB.PCI0.GFX0.DD1F, 0x87)  //Brightness down
    sleep(300) //Add delay for debounce
  }   
//[PRJ]+ <<<< implement Touch Button function   
  Method(_L0D, 0) {
