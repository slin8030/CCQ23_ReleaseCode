...
VOID
ForceChangeBootOrder (
  VOID
  )
{
  EFI_STATUS      Status;
  UINT8           BootTypeOrder = BBS_UNKNOWN;
  UINT16          *BootOrder = NULL;
  UINTN           BootOrderSize = 0;
  UINT16          TempBootOrder[16] = {0};
  UINT8           Index;
  UINT8           Index2;
  UINT8           TempBootOrderIndex;
  UINT16          BootOptionNum;
  BOOLEAN         IsExisted;
  KERNEL_CONFIGURATION *SetupConfig = NULL;
  EFI_GUID             SystemConfigurationGuid = SYSTEM_CONFIGURATION_GUID;
  UINTN                SetupVarSize;
  UINT8                TempBootTypeOrder[MAX_BOOT_ORDER_NUMBER] = {0};
  UINT8                TempBootTypeOrderIndex = 0;
 ...
 VariableSize = 0;
  ...
  for (Index = 0; Index < (BootOrderSize / sizeof (UINT16)); Index++) {
    //
    // Get current index boot option number.
    //
    BootOptionNum = BootOrder[Index];
  ...
  WriteCmos8 (0x77, 0x33);
        ...
        //
        if (IsUefiNetworkBootOption (BootOptionNum)) {
          TempBootOrder[TempBootOrderIndex] = BootOptionNum;
          TempBootOrderIndex++;
        }
        BootTypeOrder = BBS_BEV_DEVICE;
        break;
  ...
  for (Index = 0; Index < (BootOrderSize / sizeof (UINT16)); Index++) {
    ...
    IsExisted = FALSE;
    for (Index2 = 0; Index2 < TempBootOrderIndex; Index2++) {
      if (BootOptionNum == TempBootOrder[Index2]) {
        IsExisted = TRUE;
        break;
      }
    }
  ...
  //
  // Change Boot Type Order. (change BBS_BEV_DEVICE to first)
  //
  TempBootTypeOrderIndex = 0;
  TempBootTypeOrder[TempBootTypeOrderIndex] = BootTypeOrder;
  TempBootTypeOrderIndex++;
  for (Index = 0; Index < MAX_BOOT_ORDER_NUMBER; Index++) {
    if (SetupConfig->BootTypeOrder[Index] == BootTypeOrder) {
      continue;
    }
    TempBootTypeOrder[TempBootTypeOrderIndex] = SetupConfig->BootTypeOrder[Index];
    TempBootTypeOrderIndex++;
  }
  gBS->CopyMem (&SetupConfig->BootTypeOrder, &TempBootTypeOrder,  MAX_BOOT_ORDER_NUMBER);
