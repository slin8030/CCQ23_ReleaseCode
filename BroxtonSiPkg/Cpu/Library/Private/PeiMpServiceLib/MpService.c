...
#include <Library/Private/PeiCpuInitLib/Features.h>
   ...
   )
{
  ...
  PostCode (0xC3B); // PEI_INTI_CPU_DATA_HUB_START
  ...
  PostCode (0xC3E); // PEI_INTI_CPU_DATA_HUB_END
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  PostCode (0xC20); // PEI_SETUP_MP_SERVICE_ENV
...
#ifdef FSP_FLAG
  ///
  /// Don't continue the MP Initialization if SkipMpInit Token is ON.
  ///
  if (FspsUpd->FspsConfig.SkipMpInit == 1) {
    return EFI_SUCCESS;
  }
#endif
  ///
  ...
  ///
//[-start-160620-IB07400745-modify]//
  Status = WakeUpAPs (MtrrValues, (EFI_AP_PROCEDURE) InitialMpProcedure);
  if (EFI_ERROR (Status)) {
    CHIPSET_POST_CODE (PEI_MP_WAKE_UP_AP_ERROR);
    IoWrite8 (0xCF9, 0xE);
    while (TRUE);
  }
//[-end-160620-IB07400745-modify]//
  ...
  PostCode (0xC9F); // PEI_CPU_MP_SERVICE_END
  return EFI_SUCCESS;
}
...
//[-end-170410-IB07400857-add]//
  ...
  ///
//[-start-160620-IB07400745-modify]//
  Status = WakeUpAPs (MtrrValues, (EFI_AP_PROCEDURE) FutureBspProc);
  if (EFI_ERROR (Status)) {
    CHIPSET_POST_CODE (PEI_MP_WAKE_UP_AP_ERROR);
    IoWrite8 (0xCF9, 0xE);
    while (TRUE);
...
//
// @todo: Enable BIST synched with right CpuNumber for AP later on.
//
//  FillInProcessorInformation (FALSE, 0);
}
  ...
  )
{
  ...
  ACPI_CPU_DATA          *AcpiCpuData;
//[-start-160620-IB07400745-add]//
  UINTN                  WakeUpApWaitCount;
  UINTN                  WakeUpApRetry;
  BOOLEAN                WakeUpApDone;
//[-end-160620-IB07400745-add]//
...
//[-start-160620-IB07400745-modify]//
  ...
  do {
    ...
    /// Wait for all APs to complete
    ///
    WakeUpApWaitCount = 10; // 1ms
    while ((ExchangeInfo->FinishedCount < mMpCpuRuntimeData->AcpiCpuData.NumberOfCpus - 1) && WakeUpApWaitCount) {
      CpuPause ();
      MicroSecondDelay (100 * STALL_ONE_MICRO_SECOND); ///< 100us
      WakeUpApWaitCount--;
    }
    if (WakeUpApWaitCount != 0) {
      WakeUpApDone = TRUE;
    } else {
      CHIPSET_POST_CODE (PEI_WAIT_MP_WAKE_UP_APS);
//      CHIPSET_POST_CODE ((UINT32)WakeUpApRetry);
    }
  } while ((--WakeUpApRetry) && (!WakeUpApDone));
  ...
  )
{
  ...
  //CopyMem (Features, mMpSystemData->LeastFeatureProcessor.Features, sizeof (Features));
  //StartupAllAps ((EFI_AP_PROCEDURE) GetProcessorCommonFeature, NULL);

  ///
  /// Go through all processors to find the processor with least features
  ///
  //mMpSystemData->LeastFeatureProcessor.FeatureDelta = GetProcessorFeatureDelta (Features, mMpSystemData->LeastFeatureProcessor.Features);
  //StartupAllAps ((EFI_AP_PROCEDURE) GetProcessorWithLeastFeature, NULL);

  ///
  /// Switch to New BSP if current BSP is not a CPU with least processor feature
  ///
  //NewBsp = mMpSystemData->LeastFeatureProcessor.Index;

  //if (NewBsp != CurrentProcessor) {

  //  DEBUG ((DEBUG_INFO, "Switch BSP from %d ==> %d\n", CurrentProcessor, NewBsp));
  //  Status = SwitchBsp (NewBsp, TRUE);
  //}
  return EFI_SUCCESS;
}
