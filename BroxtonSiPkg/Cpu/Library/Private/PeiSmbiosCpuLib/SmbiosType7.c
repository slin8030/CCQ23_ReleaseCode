  ...
  )
{
  ...
  for (CacheIndex = 0;; CacheIndex++) {
    ...
    CacheLevel[CacheIndex]  = (UINT8) (CpuidRegisters.RegEax & V_CPUID_CACHE_LEVEL_MASK) >> B_CPUID_CACHE_LEVEL_SHIFT;
    LxCacheType[CacheIndex] = (UINT8) (CpuidRegisters.RegEax & V_CPUID_CACHE_TYPE_MASK);

    Ways[CacheIndex]        = ((CpuidRegisters.RegEbx >> B_CPUID_CACHE_PARAMS_WAYS_SHIFT) & 0x3FF) + 1;
  ...
  for (CacheIndex = 0; CacheIndex < NumberOfCacheLevels; CacheIndex++) {
    ...
    SmbiosCacheInfoData.MaxCacheSize = (UINT16)MaxCacheSize[CacheIndex];
    SmbiosCacheInfoData.InstalledSize = (UINT16)MaxCacheSize[CacheIndex];
