...
//[-start-170517-IB07400866-modify]//
    DEBUG ((EFI_D_INFO, "GPIO pad (%x)is not owned by host! skip it. Ownership: %02X\n", (Gpio_Conf_Data.CommAndOffset), Ownership));
  ...
  )
{
  ...
  *InvertState = (PadConfig0 & B_GPIO_RXINV) >> N_GPIO_RXINV;
  ...
  )
{
  ...
  PadConfig0 = GpioPadRead (GpioPad);
  PadConfig0 &= ~B_GPIO_RXINV;
  PadConfig0 |= (Value << N_GPIO_RXINV);
  ...
  )
{
  ...
  for (GroupIndex = 0; GroupIndex < GpioGroupInfoLength; GroupIndex++) {
    ...
    GpiNum = GpiNum - GpioGroupInfo[GroupIndex].PadPerGroup;
  }
  ...
  SmiRegPadConf0 = GpioRead(GpioGroupInfo[GroupIndex].Community, SmiRegOffset);
  SmiRegPadConf0 |= (1 << GpiSmiBitOffset);
  Status = GpioWrite(GpioGroupInfo[GroupIndex].Community, SmiRegOffset, SmiRegPadConf0);
  ...
  )
{
  ...
  *GpiNum       = 0;
  ...
  DEBUG ((DEBUG_INFO, "Gpio PadNumber: 0x%x \n", PadNumber));
  ...
  )
{
  ...
  *GpiSmiBitOffset = (UINT8)PadNumber;
  DEBUG((DEBUG_INFO,"PadNumber/GpiSmiBitOffset:0x%x\n", *GpiSmiBitOffset));
...
//[-start-171227-IB07400935-add]//
#ifdef GPIO_HEADER_FORMAT
BOOLEAN
GpioWakeCheck (
  IN UINT8     Community,
  IN UINT16    Index
...
//[-start-171217-IB07400935-modify]//
#ifdef GPIO_HEADER_FORMAT
  community  = (UINT8) (CommAndOffset >> 16);
  index = (UINT16)(((CommAndOffset & 0xFFFF) - GPIO_PADBAR) / 8);

  D32 = GpioRead(community, HOSTSW_OWNERSHIP_0 + (index/32)*4);
  if (D32 & (UINT32)(1 << (index%32))) {
    IsGpioDriver = TRUE; //GPIO_D
  } else {
    IsGpioDriver = FALSE; //ACPI_D
  }
  
  if ((pad_conf0.r.PMode == 0) && (pad_conf0.r.GPIORxTxDis==1)) { // GPI
    if (GpioWakeCheck (community, index)) {
      IsGpioWakeEn = TRUE; // Wake_Enabled
    } else {
      IsGpioWakeEn = FALSE; // Wake_Disabled
    }
  } else {
    IsGpioWakeEn = FALSE; // Wake_Disabled
  }

  DEBUG ((EFI_D_INFO, "BXT_GPIO_PAD_CONF(L\"%s\",", PadName));
  DEBUG ((EFI_D_INFO, " M%1d, %s, %s, %s, %s, %s, %s, %s, %s, %s, ",
    pad_conf0.r.PMode,
    (pad_conf0.r.GPIORxTxDis==0)?L"NA, NA":((pad_conf0.r.GPIORxTxDis==1)?(IsGpioDriver?L"GPI, GPIO_D":L"GPI, ACPI_D"):((pad_conf0.r.GPIORxTxDis==2)?L"GPO, NA":L"HI_Z, NA")),
    (pad_conf0.r.GPIORxTxDis==2)?((pad_conf0.r.GPIOTxState==1)?L"HI":L"LO"):L"NA",
    (pad_conf0.r.RxEvCfg==0)?L"Level":((pad_conf0.r.RxEvCfg==1)?L"Edge":((pad_conf0.r.RxEvCfg==2)?L"NA":L"Both")),
    (IsGpioWakeEn?L"Wake_Enabled":L"Wake_Disabled"),
    (pad_conf1.r.Term==0||pad_conf1.r.Term==8)?L"P_NONE":((pad_conf1.r.Term==2)?L"P_5K_L":((pad_conf1.r.Term==4)?L"P_20K_L":((pad_conf1.r.Term==9)?L"P_1K_H":((pad_conf1.r.Term==11)?L"P_2K_H":((pad_conf1.r.Term==10)?L"P_5K_H":((pad_conf1.r.Term==12)?L"P_20K_H":((pad_conf1.r.Term==13)?L"P_1K2K_H":((pad_conf1.r.Term==15)?L"Native_control":L"NA")))))))),
    (pad_conf0.r.RXINV==1)?L"Inverted":L"NA",
    (pad_conf0.r.GPIRout==0)?L"NA":((pad_conf0.r.GPIRout==1)?L"NMI":((pad_conf0.r.GPIRout==2)?L"SMI":((pad_conf0.r.GPIRout==4)?L"SCI":((pad_conf0.r.GPIRout==8)?L"IOAPIC":L"NA")))),
  ...
  DEBUG((EFI_D_INFO, "GPIO_PADBAR+0x%04x, %s),\n", 
    (CommAndOffset % 0x10000) - 0x500, 
    ((CommAndOffset >> 16)==0xC5)?L"NORTH":(((CommAndOffset >> 16)==0xC4)?L"NORTHWEST":(((CommAndOffset >> 16)==0xC7)?L"WEST":(((CommAndOffset >> 16)==0xC0)?L"SOUTHWEST":L"SOUTH")))
    ));
...
//[-start-161108-IB07400810-modify]//
  ...
  LastCommunity = Gpio_Conf_Data[0].Community;
  PrintGpioPadTitle();
  
  for (index = 0; index < Gpio_Pin_Size; index++) {
    if ((index != 0) && (LastCommunity != Gpio_Conf_Data[index].Community)) {
      PrintCommunity(Gpio_Conf_Data[index].Community);
      LastCommunity = Gpio_Conf_Data[index].Community;
      PrintGpioPadTitle();
    }
