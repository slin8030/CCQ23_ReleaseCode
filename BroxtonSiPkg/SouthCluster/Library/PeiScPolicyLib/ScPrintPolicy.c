  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, " Interrupt Polarity Control:\n"));
  for (Index = 0; Index < SC_NUM_IPC_REG; Index++) {
    DEBUG ((DEBUG_INFO, "IPC[%x] = %x \n", Index, InterruptConfig->IPC[Index]));
  }
}
  ...
  )
{
  ...
  for (Index = 0; Index < GetScXhciMaxUsb2PortNum (); Index++) {
    ...
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.PerPortTxPeHalf  = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.PerPortTxPeHalf));
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.PerPortPeTxiSet  = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.PerPortPeTxiSet));
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.PerPortTxiSet    = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.PerPortTxiSet));
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.HsSkewSel        = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.HsSkewSel));
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.IUsbTxEmphasisEn = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.IUsbTxEmphasisEn));
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.PerPortRXISet    = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.PerPortRXISet));
    DEBUG ((DEBUG_INFO, " PortUsb20[%d].AfeConfig.HsNpreDrvSel     = %x\n", Index, UsbConfig->PortUsb20[Index].AfeConfig.HsNpreDrvSel));
  }
  ...
  )
{
  ...
  for (i = 0; i < SC_MAX_SATA_PORTS; i++) {
    ...
    DEBUG ((DEBUG_INFO, " PortSettings[%d] DisableDynamicPg = %x\n", i, SataConfig->PortSettings[i].DisableDynamicPg));
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, " I2sConfiguration             = %x\n", HdaConfig->I2sConfiguration));
}
