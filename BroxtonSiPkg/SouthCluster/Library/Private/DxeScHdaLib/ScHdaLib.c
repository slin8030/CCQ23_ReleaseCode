  ...
  )
{
  ...
  if(FormatsBitmask & B_HDA_2CH_48KHZ_24BIT_FORMAT) {
    ...
    if(Format != NULL){
      ...
      if((EndpointType == HdaI2sRenderHP) || (EndpointType == HdaI2sRenderSKP)){
        if (SocSeries == BxtP) {
          if(HdaConfig->I2sConfiguration == 1) {
          ...
          if(HdaConfig->I2sConfiguration == 2) {
            Format->FormatConfiguration.CapabilitiesSize = I2sFormatConfigRenderSize_Rtk286S2ch48kHz;
            CopyMem(Format->FormatConfiguration.Capabilities, I2sFormatConfigRender_Rtk286S2ch48kHz, I2sFormatConfigRenderSize_Rtk286S2ch48kHz);
          }
        } else {
        ...
        if (SocSeries == BxtP) {
          ...
          if(HdaConfig->I2sConfiguration == 2) {
            Format->FormatConfiguration.CapabilitiesSize = I2sFormatConfigCaptureSize_Rtk286S2ch48kHz;
            CopyMem(Format->FormatConfiguration.Capabilities, I2sFormatConfigCapture_Rtk286S2ch48kHz, I2sFormatConfigCaptureSize_Rtk286S2ch48kHz);
          }
        } else {
  ...
  if(FormatsBitmask & B_HDA_I2S_RTK286S_2CH_48KHZ_24BIT_FORMAT) {
    ...
    Format = GetNhltEndpointFormat (Endpoint, FormatIndex++);
    if(Format != NULL){
      CopyMem (&(Format->Format), &Ch2_48kHz24bitFormat16sample, sizeof (WAVEFORMATEXTENSIBLE));
  ...
  )
{
  ...
  switch(EndpointType) {
    ...
    case HdaI2sRenderSKP:
      ...
      if (HdaConfig->I2sConfiguration == 2) {
        //
        // For BXT-P, Virtual Bus ID is 1, while it is 0 for BXTM
        //
        if (SocSeries == BxtP) {
          HdaEndpointI2sRenderSKP.VirtualBusId = 1;
          DEBUG ((DEBUG_INFO, "For BXT-P, Endpoint: HdaI2sRenderSKP virtual bus ID is 1.\n"));
        }
        CopyMem (Endpoint, &HdaEndpointI2sRenderSKP, sizeof (ENDPOINT_DESCRIPTOR));
        EndpointConfigBuffer     = I2sConfigSKP_Rtk286S2ch48kHz;
        EndpointConfigBufferSize = I2sConfigSKPSize_Rtk286S2ch48kHz;
        EndpointFormatsBitmask   = B_HDA_2CH_48KHZ_24BIT_FORMAT | B_HDA_I2S_RTK286S_2CH_48KHZ_24BIT_FORMAT;
      }
      break;
    case HdaI2sCaptureSKP:
      ...
      if (HdaConfig->I2sConfiguration == 2) {
        ...
        //
        if (SocSeries == BxtP) {
          HdaEndpointI2sCapture.VirtualBusId = 1;
          DEBUG ((DEBUG_INFO, "For BXT-P, Endpoint: HdaI2sCaptureSKP virtual bus ID is 1.\n"));
        }
        CopyMem (Endpoint, &HdaEndpointI2sCapture, sizeof (ENDPOINT_DESCRIPTOR));
        EndpointConfigBuffer     = I2sConfig;
        EndpointConfigBufferSize = I2sConfigSize;
    ...
    case HdaI2sRenderHP:
      ...
      if(HdaConfig->I2sConfiguration == 2) {
        //
        // For BXT-P, Virtual Bus ID is 3, while it is 0 for BXTM
        //
        if (SocSeries == BxtP) {
          HdaEndpointI2sRenderHP.VirtualBusId = 3;
          DEBUG ((DEBUG_INFO, "For BXT-P, Endpoint: HdaI2sRenderHP virtual bus ID is 3.\n"));
        }
        CopyMem (Endpoint, &HdaEndpointI2sRenderHP, sizeof (ENDPOINT_DESCRIPTOR));
        EndpointConfigBuffer     = I2sConfig;
        EndpointConfigBufferSize = I2sConfigSize;
        EndpointFormatsBitmask   = B_HDA_I2S_RTK286S_2CH_48KHZ_24BIT_FORMAT | B_HDA_2CH_48KHZ_24BIT_FORMAT;
      }
      break;
    case HdaI2sCaptureHP:
      ...
      if (HdaConfig->I2sConfiguration == 2) {
        ...
        //
        if (SocSeries == BxtP) {
          HdaEndpointI2sCapture.VirtualBusId = 3;
          DEBUG ((DEBUG_INFO, "For BXT-P, Endpoint: HdaI2sCaptureHP virtual bus ID is 3.\n"));
        }
        CopyMem (Endpoint, &HdaEndpointI2sCapture, sizeof (ENDPOINT_DESCRIPTOR));
        EndpointConfigBuffer     = I2sConfig;
        EndpointConfigBufferSize = I2sConfigSize;
  ...
  EndpointDescriptorLength += NhltFormatsConstructor (Endpoint,EndpointFormatsBitmask,EndpointType, ScPolicy);
  ...
  )
{
  ...
  for (Index = 0; Index < HdaEndpointMax; Index++) {
    if (EndpointTable[Index].Enable == TRUE) {
     EndpointDescriptorsLength += NhltEndpointConstructor (Table, EndpointTable[Index].EndpointType, Table->EndpointCount++, ScPolicy);
    }
  }
