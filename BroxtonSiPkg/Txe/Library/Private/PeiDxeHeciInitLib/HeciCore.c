...
BOOLEAN  mRecoveryModeEnabled = FALSE;
...
//[-start-190322-IB07401094-modify]//
...
//[-start-160531-IB07220089-add]//
//[-start-160712-IB07220113-modify]//
  if (mRecoveryModeEnabled == TRUE) {
    MaxCount = CSE_WAIT_TIMEOUT;
  }
//[-end-160712-IB07220113-modify]//
//[-end-160531-IB07220089-add]//
...
//[-start-160807-IB07400769-add]//
  if (!IsTxeDataRegionAvailable()) {
    *SeCMode = SEC_MODE_RECOVER;
    DEBUG ((EFI_D_INFO, "TXE data region unavailabe!!\n"));
    return EFI_SUCCESS;
  }
...
//[-start-160804-IB07400769-add]//
//
// Check TXE data region available or not
// Return:
//   TRUE  - Available
//   FALSE - FDO mode or Recovery or other 
//
BOOLEAN
  ...
  )
{
  ...
  if (mFdoModeEnabled) { // FDO mode, TXE Unavailable
    return FALSE;
  }
  
  CmosData = ReadExtCmos8 (R_XCMOS_INDEX, R_XCMOS_DATA, CmosChipsetFeature2); 
  if (CmosData & B_CMOS_TXE_DATA_UNAVAILABLE) { // TXE Data Region Unavailable (During Update BIOS)
    return FALSE;  
  } 
