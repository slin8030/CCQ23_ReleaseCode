...
CRYPTO_SERVICES_PROTOCOL *mCrypto;
//[-end-160808-IB07220123-add]//
//[-start-160816-IB07220129-add]//
MBP_SMM_TRUSTED_KEY      *mSmmTrustedKey = NULL;
...
ComputeHmacSha256Signature (
  ...
  ContextSize = mCrypto->HmacSha256GetContextSize ();
  ...
  Status  = mCrypto->HmacSha256Init (HmacContext, mHmacSha256Key, sizeof (mHmacSha256Key));
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "[Fail]"));
    return EFI_ABORTED;
  }
  ...
  Status  = mCrypto->HmacSha256Update (HmacContext, Data, DataSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "[Fail]"));
    return EFI_ABORTED;
  }
  ...
  Status  = mCrypto->HmacSha256Final (HmacContext, Digest);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "[Fail]"));
    return EFI_ABORTED;
  }
...
//[-start-160816-IB07220129-add]//
  if (mSmmTrustedKey != NULL) {
    mMonoCount = ++mSmmTrustedKey->MonotonicCounter;
    if (CompareMem (mHmacSha256Key, mSmmTrustedKey->SmmTrustedKey, sizeof (mHmacSha256Key))) {
      CopyMem (mHmacSha256Key, mSmmTrustedKey->SmmTrustedKey, sizeof (mHmacSha256Key));
    }
  }
//[-end-160816-IB07220129-add]//
...
//[-start-170613-IB07400874-modify]//
  DEBUG ((EFI_D_INFO, "BIOS Start Send HECI Message: Heci2WriteNVMFile (FileName:%a)\n", FileName));
...
//[-start-170613-IB07400874-modify]//
  DEBUG ((DEBUG_INFO, "BIOS Start Send HECI Message: Heci2ReadNVMFile (FileName:%a)\n", FileName));
//[-end-170613-IB07400874-modify]//
...
//[-start-170613-IB07400874-modify]//
  DEBUG ((EFI_D_INFO, "BIOS Start Send HECI Message: Heci2GetNVMFileSize (FileName:%a)\n", FileName));
//[-end-170613-IB07400874-modify]//
...
// /**
//   Send Write NVM file message through HECI2 but not waiting for response.
// 
//   @param[in] FileName   The file name.
//   @param[in] Offset        The offset of data.
...
//   @param[in] Heci2Protocol The HECI protocol to send the message to HECI2 device.
// 
//   @return EFI_SUCCESS   Write file success.
//   @return Others              Write file failed.
// **/
// EFI_STATUS
...
// 
//   if (DataSize > MAX_HECI2_WRITE_DATA_SIZE) {
//     Status = Heci2WriteNVMFileNoResp(FileName, Offset, Data, MAX_HECI2_WRITE_DATA_SIZE, Heci2Protocol);
//     if (EFI_ERROR(Status)) {
//       return Status;
//     }
//     Status = Heci2WriteNVMFileNoResp(FileName, Offset + MAX_HECI2_WRITE_DATA_SIZE, Data + MAX_HECI2_WRITE_DATA_SIZE, DataSize - MAX_HECI2_WRITE_DATA_SIZE, Heci2Protocol);
//     if (EFI_ERROR(Status)) {
//       return Status;
//     }
//     return EFI_SUCCESS;
...
// 
//   WriteFileMessage->Offset   = Offset;
//   WriteFileMessage->DataSize = (UINT16)DataSize;
//   WriteFileMessage->Truncate = 0;
// 
//   WriteFileMessage->TrustedChannelHeader.CommandId = HECI2_WRITE_DATA_CMD_ID;
// 
//   //
...
// 
//   CopyMem ((UINT8*) Heci2DataBuffer + sizeof (HECI2_TRUSTED_CHANNEL_BIOS_WRITE_REQ), Data, DataSize);
// 
//   //
//   // Fill in the HMAC signature and update the monotonic counter
//   //
...
//   }
// 
//   HeciSendLength = (UINT32) (sizeof (HECI2_TRUSTED_CHANNEL_BIOS_WRITE_REQ) + DataSize);
//   HeciRecvLength = sizeof (HECI2_TRUSTED_CHANNEL_BIOS_WRITE_RESP);
// 
//   Status = Heci2Protocol->SendMsg (
//              HECI2_DEVICE,
  ...
  )
{
  ...
  if (!InSmm) {
    ...
    //
    // Locate Crypto Service protocol for SMM
    //
