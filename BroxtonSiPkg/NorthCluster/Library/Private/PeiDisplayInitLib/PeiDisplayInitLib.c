...
//[-start-191206-IB16530064-add]//
  ...
  if (BootMode == BOOT_IN_RECOVERY_MODE) {
    //
    //Use CMOS 0x19 to check is it first time enter PeiDisplayInit, if initial twice time it would hang. 
    //
    CMosData = ReadExtCmos8 (R_XCMOS_INDEX, R_XCMOS_DATA, 0x19);
    if (CMosData == 1) {
      DEBUG ((DEBUG_INFO, "PeiDisplayInit again skip initial\n"));
      CMosData = 0;
      WriteExtCmos8 (R_XCMOS_INDEX, R_XCMOS_DATA, 0x19, CMosData);
      return EFI_SUCCESS;
    } else {
      DEBUG ((DEBUG_INFO, "PeiDisplayInit first time\n"));
      CMosData = 1;
      WriteExtCmos8 (R_XCMOS_INDEX, R_XCMOS_DATA, 0x19, CMosData);
    }
  }
