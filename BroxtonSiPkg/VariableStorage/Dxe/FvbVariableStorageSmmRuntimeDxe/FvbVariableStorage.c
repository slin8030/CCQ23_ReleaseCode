...
// Module globals
//
VARIABLE_STORAGE_PROTOCOL mFvbVariableStorageProtocol = {
  ...
  FvbVariableStorageRegisterWriteServiceReadyCallback,
...
///
/// Function pointer to invoke after the VARIABLE_STORAGE_PROTOCOL is ready to accept write requests
///
VARIABLE_STORAGE_WRITE_SERVICE_READY_CALLBACK mWriteServiceReadyCallback = NULL;
  ...
  )
{
  ...
  Status = EFI_SUCCESS;
  if ((mWriteServiceReadyCallback != NULL) && (CallbackFunction == NULL)) {
    return EFI_INVALID_PARAMETER;
  }
  mWriteServiceReadyCallback = CallbackFunction;
  if (mFvbInstance != NULL) {
    Status = mWriteServiceReadyCallback ();
    if (!EFI_ERROR (Status)) {
      mWriteServiceReadyCallback = NULL;  //Don't call it twice if successful
    }
  }
