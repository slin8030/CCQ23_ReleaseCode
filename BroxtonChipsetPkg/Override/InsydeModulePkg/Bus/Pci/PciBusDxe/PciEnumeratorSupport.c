  ...
  )
{
  ...
  // Do not assign a PCI BAR to P2SB, PMC-IPC, or PMC-SSRAM.
  // These devices are accessed via APCI and will conflict with PCI resources listed in HostBus.asl
  // This w/a should be moved to a better location but is here now to enable the OS drivers.
  if((Bus == 0x00) && (Device == 0xD)) {
    DEBUG ((EFI_D_INFO, "PCI Enum skip - PciBus: PCI @ [%02x|%02x|ALL] is skipped\n", Bus, Device));
...
//[-end-160819-IB07400772-modify]//
  if((Bus == 0x00) && (Device == 24) && (Func == 2)) {
    DEBUG ((EFI_D_INFO, "PCI Enum skip - PciBus: PCI @ [%02x|%02x|%02x] is skipped\n", Bus, Device, Func));
    return EFI_NOT_FOUND;
  }
#endif  
  ...
  )
{
  ...
  PCI_SET_TEST_COMMAND_REGISTER (PciIoDevice, *Command);
  PCI_READ_COMMAND_REGISTER (PciIoDevice, Command);

  //
  // Write back the original value
  //
  PCI_SET_TEST_COMMAND_REGISTER (PciIoDevice, *OldCommand);
  ...
  if (IS_PCI_BRIDGE (&PciIoDevice->Pci) || IS_CARDBUS_BRIDGE (&PciIoDevice->Pci)) {
    ...
    PCI_SET_TEST_BRIDGE_CONTROL_REGISTER (PciIoDevice, *BridgeControl);
    PCI_READ_BRIDGE_CONTROL_REGISTER (PciIoDevice, BridgeControl);

    //
    // Write back the original value
    //
    PCI_SET_TEST_BRIDGE_CONTROL_REGISTER (PciIoDevice, *OldBridgeControl);
  ...
  )
{
  ...
  if ((Value & 0x01) != 0) {
    ...
    case 0x04:
      ...
      //
      // shl UINT32 with 32 counts is the undefined behavior.
      //
      Value |= LShiftU64 ((UINT64)(-1), Index);
