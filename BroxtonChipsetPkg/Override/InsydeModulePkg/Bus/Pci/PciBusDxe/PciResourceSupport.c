...
typedef struct {
  ...
  UINT8  Dev;
  UINT8  Fun;
} SKIP_64BIT_MMIO_TABLE;

//
// Default skip all Onboard PCI device (Please comment out for support)
//
SKIP_64BIT_MMIO_TABLE mSkip64bitMmioTable[] = {
  // Bus,  Dev, Fun
  {  0x0, 0x0E, 0x0 }, // Audio
  {  0x0, 0x11, 0x0 }, // ISH  M64/P64
  {  0x0, 0x15, 0x1 }, // XDCI
  {  0x0, 0x16, 0x0 }, // I2C0 M64/P64
  {  0x0, 0x16, 0x1 }, // I2C1 M64/P64
  {  0x0, 0x16, 0x2 }, // I2C2 M64/P64
  {  0x0, 0x16, 0x3 }, // I2C3 M64/P64
  {  0x0, 0x17, 0x0 }, // I2C4 M64/P64
  {  0x0, 0x17, 0x1 }, // I2C5 M64/P64
  {  0x0, 0x17, 0x2 }, // I2C6 M64/P64
  {  0x0, 0x17, 0x3 }, // I2C7 M64/P64
  {  0x0, 0x18, 0x0 }, // UART0 M64/P64
  {  0x0, 0x18, 0x1 }, // UART1 M64/P64
  {  0x0, 0x18, 0x3 }, // UART3 M64/P64
  {  0x0, 0x19, 0x0 }, // SPI0 M64/P64
  {  0x0, 0x19, 0x1 }, // SPI1 M64/P64
  {  0x0, 0x19, 0x2 }, // SPI2 M64/P64
  {  0x0, 0x1A, 0x0 }, // PWM
  {  0x0, 0x1D, 0x0 }, // USF
  {  0x0, 0x1E, 0x0 }, // SDIO M64/P64
  {  0x0, 0x1F, 0x1 }  // SMBUS M64
};

SKIP_64BIT_MMIO_TABLE m64bitUnSupportTable[] = {
  // Bus,  Dev, Fun
  {  0x0, 0x02, 0x0 }, // IGD, M64/P64
  {  0x0, 0x0D, 0x0 }, // P2SB M64
  {  0x0, 0x0D, 0x1 }, // PMC  M64/P64
  {  0x0, 0x0D, 0x3 }, // Shared SRAM M64/P64
  {  0x0, 0x0F, 0x0 }, // HECI1 M64
  {  0x0, 0x0F, 0x1 }, // HECI2 M64
  {  0x0, 0x0F, 0x2 }, // HECI2 M64
  {  0x0, 0x15, 0x0 }, // XHCI M64
  {  0x0, 0x18, 0x2 }, // UART2 M64/P64
  {  0x0, 0x1B, 0x0 }, // SD card M64/P64
  {  0x0, 0x1C, 0x0 }, // eMMC M64/P64
};

UINTN mSkip64bitMmioTableSize = sizeof (mSkip64bitMmioTable) / sizeof (SKIP_64BIT_MMIO_TABLE);
UINTN m64bitUnSupportTableSize = sizeof (m64bitUnSupportTable) / sizeof (SKIP_64BIT_MMIO_TABLE);

BOOLEAN
IsSkipPciDeviceFor64bitMmio (
  IN PCI_IO_DEVICE        *Temp
  )
{
  UINTN  Index;
  
  for (Index = 0; Index < m64bitUnSupportTableSize; Index ++) {
    if ((Temp->BusNumber == m64bitUnSupportTable[Index].Bus) &&
        (Temp->DeviceNumber == m64bitUnSupportTable[Index].Dev) &&
        (Temp->FunctionNumber == m64bitUnSupportTable[Index].Fun)) {
      return TRUE;
    }
  }
  
  if ((PcdGet8 (PcdMmioMem64SkipOnboard) == 0x0)) {
    return FALSE;
  }

  for (Index = 0; Index < mSkip64bitMmioTableSize; Index ++) {
    if ((Temp->BusNumber == mSkip64bitMmioTable[Index].Bus) &&
        (Temp->DeviceNumber == mSkip64bitMmioTable[Index].Dev) &&
        (Temp->FunctionNumber == mSkip64bitMmioTable[Index].Fun)) {
      return TRUE;
    }
  }
  return FALSE;
...
#ifdef PCI_64BITS_MMIO_DECODE_SUPPORT
    //
    // WA for OS driver do not support 64-bit MMIO address, so degrade it's resources from PMem64 to PMem32
    //
    if ((PcdGet8 (PcdMmioMem64Enable) == 0x01) && (IsSkipPciDeviceFor64bitMmio(Temp))) {
