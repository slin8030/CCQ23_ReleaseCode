...
UINT64    mAutoComplete = FALSE;
UINT64    mAutoCompleteStartTime = 0;
/**
  Get HID Descriptor
...
Exit:
  ...
  mAutoCompleteStartTime = GetPerformanceCounter();
  mAutoComplete = TRUE;  
}
  ...
  )
{
  I2C_TOUCH_PANEL_DEV *TpDev;
  EFI_TPL             OldTpl;
  UINT64               AutoCompleteEndTime;
  UINT64               AutoCompleteDurationTime;
  ...
  if (!TpDev->StateChanged) {
//[-start-151209-IB06740449-modify]//
    if(mAutoComplete) { 
      AutoCompleteEndTime = GetPerformanceCounter();
      AutoCompleteDurationTime = AutoCompleteEndTime - mAutoCompleteStartTime;
      if(AutoCompleteDurationTime > 100000000u) {
        TpDev->State[TpDev->BufferOut].ActiveButtons = 0;
        CopyMem (
          TouchPanelState,
