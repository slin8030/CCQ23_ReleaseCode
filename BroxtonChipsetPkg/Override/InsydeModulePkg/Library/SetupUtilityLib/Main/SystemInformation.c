...
//[-start-170117-IB07400834-modify]//
#include <Library/MultiPlatformBaseLib.h>
#include <Guid/PlatformInfo.h>
//#endif
//[-end-170117-IB07400834-modify]//
  ...
  ) 
{
  ...
  if (SCUData == NULL) {
    Status = GetChipsetSetupVariableDxe (SCUData, sizeof(CHIPSET_CONFIGURATION));  
    ASSERT (SCUData != NULL);
  }
  if (EFI_ERROR (Status)) {
    return Status;
...
//[-start-170117-IB07400834-modify]//
  UPDATE_INFO_STRUCT          *UpdateRamSlotPtr;
  UINT8                       BoardId;
  UPDATE_INFO_STRUCT          UpdateRamSlotInfo_2_Channel[] = {
                                {1, STRING_TOKEN(STR_CHA_DIMM0_STRING), STRING_TOKEN(STR_CHA_DIMM0_STRING2)},
                                {2, STRING_TOKEN(STR_CHB_DIMM0_STRING), STRING_TOKEN(STR_CHB_DIMM0_STRING2)}
                                };

  UPDATE_INFO_STRUCT          UpdateRamSlotInfo_4_Channel[] = {
                                {1, STRING_TOKEN(STR_CHA_DIMM0_STRING), STRING_TOKEN(STR_CHA_DIMM0_STRING2)},
                                {2, STRING_TOKEN(STR_CHB_DIMM0_STRING), STRING_TOKEN(STR_CHB_DIMM0_STRING2)},
                                {3, STRING_TOKEN(STR_CHC_DIMM0_STRING), STRING_TOKEN(STR_CHC_DIMM0_STRING2)},
                                {4, STRING_TOKEN(STR_CHD_DIMM0_STRING), STRING_TOKEN(STR_CHD_DIMM0_STRING2)}
                                };

  
  BoardId = MultiPlatformGetBoardIds();
  DEBUG ((EFI_D_ERROR, "BoardId = %x\n", BoardId));
  if ((BoardId == BOARD_ID_APL_RVP_1A) || //DDR3L non-ECC
      (BoardId == BOARD_ID_OXH_CRB) || //DDR3L non-ECC
      (BoardId == BOARD_ID_JNH_CRB) // DDR3L ECC
      ) { 
    UpdateRamSlotPtr = UpdateRamSlotInfo_2_Channel;
    NumSlot = sizeof (UpdateRamSlotInfo_2_Channel) / sizeof (UPDATE_INFO_STRUCT);
    CreateRamSlotInfo (
      HiiHandle,
      NumSlot,
      UpdateRamSlotInfo_2_Channel
      );
  } else if ((BoardId == BOARD_ID_LFH_CRB) || // LPDDR4
             (BoardId == BOARD_ID_APL_RVP_2A)) { // LPDDR3
    UpdateRamSlotPtr = UpdateRamSlotInfo_4_Channel;
    NumSlot = sizeof (UpdateRamSlotInfo_4_Channel) / sizeof (UPDATE_INFO_STRUCT);
    CreateRamSlotInfo (
      HiiHandle,
      NumSlot,
      UpdateRamSlotInfo_4_Channel
      );
  } else {

    UpdateRamSlotPtr = UpdateRamSlotInfo;
...
//[-start-170117-IB07400834-remove]//
//          gBS->FreePool (NewString);
//[-end-170117-IB07400834-remove]//
        }

//[-start-170324-IB07400849-modify]//
        ZeroMem (StrBuffer, sizeof (StrBuffer));
        if (!StrnCmp (NewString, L"Channel", StrLen (L"Channel"))) {
          UnicodeSPrint (StrBuffer, 40, L"Channel %c - ", NewString[StrLen (L"Channel")]);
          SlotIndex = (UINT8)(NewString[StrLen (L"Channel")] - 'A');
        }
        StrCat (StrBuffer, mMemFormFactorStr[Type17Record->FormFactor - 1]);
        //
        // Apollo Lake CRB, 1 DIMM per channel
        //
//        if (SlotIndex % 2 == 0) {
...
//        }
        gBS->FreePool (NewString);
        TokenToUpdate = (STRING_REF) (UpdateRamSlotPtr[SlotIndex].TokenToUpdate1);
//[-end-170324-IB07400849-modify]//
        HiiSetString (HiiHandle, TokenToUpdate, StrBuffer, NULL);
      }
...
//[-start-170117-IB07400834-modify]//
        TokenToUpdate = (STRING_REF) (UpdateRamSlotPtr[SlotIndex].TokenToUpdate2);
//[-end-170117-IB07400834-modify]//
        HiiSetString (HiiHandle, TokenToUpdate, StrBuffer, NULL);
      }
      Index++;
