  ...
  )
{
  ...
  Mbr = AllocatePool (sizeof (MASTER_BOOT_RECORD));
  if (Mbr == NULL) {
    return Found;
  }
  ...
  if (LastDevicePathNode == NULL) {
    ...
    for (Index = 0; Index < MAX_MBR_PARTITIONS; Index++) {
      ...
      Status = PartitionInstallChildHandle (
                ...
                MBR_SIZE,
                (BOOLEAN) (Mbr->Partition[Index].OSIndicator == EFI_PARTITION) ? &gEfiPartTypeSystemPartGuid : NULL
                );
    ...
    do {
      ...
      *((UINT32 *) &HdDev.Signature[0]) = 0;
      ...
      Status = PartitionInstallChildHandle (
                 ...
                 MBR_SIZE,
                 (BOOLEAN) (Mbr->Partition[0].OSIndicator == EFI_PARTITION) ? &gEfiPartTypeSystemPartGuid : NULL
                 );
      if (!EFI_ERROR (Status)) {
        Found = EFI_SUCCESS;
      }
