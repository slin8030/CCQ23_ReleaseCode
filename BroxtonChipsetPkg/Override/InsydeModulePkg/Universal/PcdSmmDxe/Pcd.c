...
BOOLEAN mSmmReadyToLockFlag = FALSE;
  ...
  )
{
  ...
  PcdDbLen = mPcdDatabase.PeiDb->Length + mPcdDatabase.PeiDb->UninitDataBaseSize;
  mPcdDatabase.PeiDb = AllocateCopyPool (PcdDbLen, mPcdDatabase.PeiDb);
  PcdDbLen = mPcdDatabase.DxeDb->Length + mPcdDatabase.DxeDb->UninitDataBaseSize;
  mPcdDatabase.DxeDb = AllocateCopyPool (PcdDbLen, mPcdDatabase.DxeDb);
  mDistinctTokenSpace = (EFI_GUID **)AllocateCopyPool(TmpTokenSpaceBufferCount * sizeof (EFI_GUID *), mDistinctTokenSpace);
  TmpTokenSpaceBuffer = (EFI_GUID **)AllocateCopyPool(TmpTokenSpaceBufferCount * sizeof (EFI_GUID *), TmpTokenSpaceBuffer);
  ...
  )
{
  ...
  ZeroMem (mDistinctTokenSpace, TmpTokenSpaceBufferCount * sizeof (EFI_GUID *));
  TsIdx = 0;
