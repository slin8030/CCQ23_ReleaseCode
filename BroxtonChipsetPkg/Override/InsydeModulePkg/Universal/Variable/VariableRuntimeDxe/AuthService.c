  ...
  )
{
  ...
  //
  if ((Data == NULL) || (DataSize == 0) || (mCryptoService == NULL)) {
    return EFI_SUCCESS;
  }
...
/**
  This function uses to find secure boot database default variable header in
  factory default region.

  @param[in]  VariableName       Name of Variable to be found.
  @param[in]  VendorGuid         Variable vendor GUID.

  @return     Pointer to variable header or NULL if not found.

**/
VARIABLE_HEADER *
FindSecureDatabaseDefaultVariableHeader (
  IN      CHAR16            *VariableName,
  IN      EFI_GUID          *VendorGuid
  )
{
  ...
  )
{
  ...
  VariableHeader = FindSecureDatabaseDefaultVariableHeader (VariableName, VendorGuid);

  if (VariableHeader == NULL && !IsDbxDefaultVariable (VariableName, VendorGuid)) {
    return EFI_NOT_FOUND;
  }
