...
//[-start-160919-IB15550021-add]//
/**
  Read and update SPD Module Part Number through SMBUS protocol.
  
  @param[in]  Dimm              - the number of the current Dimm.
  @param[out] *PartNumber       - the buffer which stores the information read from SPD.

  @retval EFI_SUCCESS           - if the data is successfully reported.
  @retval EFI_NOT_FOUND         - if no protocol instances were found.
  @retval EFI_INVALID_PARAMETER - if a required parameter in a subfunction is NULL.
**/
EFI_STATUS
SpdDdr3ModulePartNumberRead (
  IN  UINT8 Dimm,
  OUT CHAR8 *PartNumber
  ) 
{
  EFI_STATUS                      Status;
  ...
  UINT8                           SpdAddress;
  UINTN                           PNStringLength;
  EFI_HOB_GUID_TYPE               *GuidHob;
  MRC_NV_DATA_FRAME               *MemoryConfigHobData;

  PNStringLength = SPD_DDR3_MODULEPN_STRING_LEN;
  
  GuidHob = NULL;
  MemoryConfigHobData = NULL;
  ...
  SpdAddress = MemoryConfigHobData->MrcParamsSaveRestore.DramPolicyData.SpdAddress[Dimm];
  ...
  //
  // Address[7:0] = SlaveAddress(Bit[7:1]) + Read/Write(Bit[0])
  // Bit[0] must be set through EFI_SMBUS_OPERATION (i.e. EfiSmbusReadByte, EfiSmbusWriteByte...)
  //
  SmbusSlaveAddress.SmbusDeviceAddress = SMBUS_LIB_SLAVE_ADDRESS (SpdAddress);
...
//[-start-160921-IB15550021-add]//
  SMBIOS_TABLE_TYPE17            *Type17Record;
  UINT8                          StringCount;
  CHAR8                          *DimmNumberString;
  CHAR8                          *PartNumberString;
  UINT8                          Dimm; 
//[-end-160921-IB15550021-add]//
  CHAR8                          *LanguageTable;
  CHAR8                          *SCULanguage;
  CHAR8                          *Type13Lang;
...
//[-start-160922-IB15550021-add]//
  Dimm = 0;
//[-end-160922-IB15550021-add]//
  ...
  for (Link = Head->ForwardLink; Link != Head; Link = Link->ForwardLink) {
    ...
    case EFI_SMBIOS_TYPE_BIOS_LANGUAGE_INFORMATION:
      ...
      // Get SCU language.
      //
      SCULanguage = GetEfiGlobalVariable (L"PlatformLang");
      if (SCULanguage == NULL) {
        SCULanguage = AllocatePool (AsciiStrSize("en-US"));
        if (SCULanguage == NULL) {
          return EFI_OUT_OF_RESOURCES;
        }
        AsciiStrCpy (SCULanguage, (CHAR8 *) "en-US");
      }
...
//[-start-160921-IB15550021-add]//
    //
    // Update Type17 Part number
    //
    case EFI_SMBIOS_TYPE_MEMORY_DEVICE:
      Type17Record = (SMBIOS_TABLE_TYPE17 *) CurrentSmbiosHeader;
      //
      // Skip over formatted section
      //
      PartNumberString = (CHAR8 *) Type17Record + sizeof (SMBIOS_TABLE_TYPE17);
      
      for (StringCount = 1; StringCount <= Type17Record->PartNumber; StringCount++) {
        if (StringCount == Type17Record->PartNumber) {
          break;
        }
        while (*PartNumberString != 0) {
          PartNumberString++;
        }
        PartNumberString++;
      }
      ...
      // Check Dimm number 
      //
      DimmNumberString = (CHAR8 *) Type17Record + sizeof (SMBIOS_TABLE_TYPE17);
      for (StringCount = 1; StringCount <= Type17Record->DeviceLocator; StringCount++) {
        while (*DimmNumberString != 0) {
          DimmNumberString++;
        }
        if (StringCount == Type17Record->DeviceLocator) {
          //
          // back to the char before null char 
          //
          DimmNumberString--;
          break;
        }
      ...
      //
      // Get Dimm number from Smbios table ('0'-> Dimm0,'1'-> Dimm1) 
      //
      switch (*DimmNumberString) {
      ...
      //
      // Read PartNumberString from SPD and update Smbios table
      //
      SpdDdr3ModulePartNumberRead (Dimm, PartNumberString);
      break;
//[-end-160921-IB15550021-add]//
    default:
      break;
