...
;[-start-160413-IB03090432-modify];
...
struc HobStruc
  ...
  .TempRamBase          RESD  1            ; PEI TempRam Base .
  .TempRamSize          RESD  1            ; PEI TempRam Size
  .HostToCse            RESD  1
  .CseToHost            RESD  1
  .ChunkIndex           RESD  1
...
;%ifdef BXTI_PF_ENABLE
  .BootPolicyExist      RESD  1            ; Found Boot Policy
  .TxeHashExist         RESD  1            ; Found TXE Measurement Hash
  .BootGuard            RESD  1            ; Boot Guard
  .TxeHash              RESB  32           ; TXE Measurement Hash
;%endif
...
%ifdef FSP_WRAPPER_FLAG      ;FSP based boot
;[-start-161123-IB07250310-add];
;                      |-------------------|--->  0xFF000000
;                      |                   |  0x2C000
;                 <----|-------------------|--->  0xFEFD4000
;                      |      | FSP-M      |  0x59000        (BootFirmwareVolumeSize) (FSP-M IBBM Size)
; 0x83000 (IBBSize)    | IBBM |------------|--->  0xFEF7B000 (BootFirmwareVolumeBase)
;                      |      | BOOTLOADER |  0x2A000        (BOOTLOADER IBBM Size)
;                 <----|-------------------|--->  0xFEF51000 (IBBBase)
;                      |       FIT         |  0x4000
; 0x05000 (Free space) |-------------------|--->  0xFEF4D000 (FITBase)
;                      |       IBBL        |  0x1000         (IBBLSize)
; |--------------------|-------------------|--->  0xFEF4C000 (IBBLBase)
; | Stack 35%          |       FSP         |
; |--------------------|                   |  0x2A000        (Fsp TemporaryRamSize) (FSP temp memory size)
; | Heap  65%          | Stack & Heap Size |        
; |--------------------|-------------------|--->  0xFEF22000
; | Stack 0x4000       |                   |
; |--------------------| Stack & Heap Size |  0x22000        (Stack&Heap Size) (BOOTLOADER temp memory size)
; | Heap  0x1C000      |                   |        
; |--------------------|-------------------|--->  0xFEF00000 (Stack&Heap Base) (Car Base Address)
;
; Fsp TemporaryRamBase       - 0xFEF22000
; Fsp TemporaryRamSize       - 0x2A000
; Fsp PeiTemporaryRamBase    - 0xFEF22000
; Fsp PeiTemporaryRamSize    - 0x1B4CC(65%)
; Fsp StackBase              - 0xFEF3D4CC
; Fsp StackSize              - 0xEB34(35%)
...
istruc HobStruc
    ...
    dd 0xFEF51000   ; .IBBBase     = .IBBLBase (above FIT), must align with FspPCDs and FV BaseAddress
    dd 0x00083000   ; .IBBSize     = FLASH_REGION_FV_IBBM_SIZE (in .fdf) + size_of FSP_M.fv
    dd 0xFFFFF000   ; .IBBLSource  = 0x100000000 - .IBBLSize = PcdFlashFvIBBLBase
    dd 0xFEF4C000   ; .IBBLBase    = .CarBase (above TempRAM)
    dd 0x00001000   ; .IBBLSize    = PcdFlashFvIBBLSize = FLASH_REGION_FV_IBBL_SIZE in .fdf
    dd 0xFEF4D000   ; .FITBase     = IBBLBase + IBBLSize
    dd 0xFEF00000   ; .TempRamBase = .CarBase
    dd 0x22000      ; .TempRamSize <= IBBLBase - CarBase
    dd 0            ; .HostToCse
    dd 0            ; .CseToHost
    dd 0            ; .ChunkIndex
    dd 0            ; .NumberOfChunks
...
; |--------------------|-------------------|--->  0xFF000000
; | Stack 0x12000      |                   |
; |--------------------| Stack & Heap Size |  0x9E800        (Stack&Heap Size) (BOOTLOADER temp memory size)
; | Heap  0x8C800      |                   |        
; |--------------------|-------------------|--->  0xFEF61800 (Stack&Heap Base) (Car Base Address)
;                      |       FIT         |  0x800
;                      |-------------------|--->  0xFEF61000 (FITBase)
;                      |       IBBL        |  0x1000
;                      |-------------------|--->  0xFEF60000 (IBBLBase)
;                      |       IBB         |  0x60000
;                 <----|-------------------|--->  0xFEF00000 (IBBBase)(Car Base Address)
;[-end-161123-IB07250310-add];
HOBStructure:
istruc HobStruc
    dd "$SIG"       ; .Sign
    dd 0xFEF00000   ; .CarBase
    dd 0x100000     ; .CarSize
    dd 0xFFFAB000   ; .IBBSource   = .IBBLSource - .IBBSize = PcdFlashFvIBBMBase (part of SKIPCSE)
    dd 0xFEF00000   ; .IBBBase     = .CarBase
    dd 0x60000      ; .IBBSize    = PcdFlashFvIBBMSize = FLASH_REGION_FV_IBBM_SIZE in .fdf
    dd 0xFFFFF000   ; .IBBLSource  = 0x100000000 - .IBBLSize = PcdFlashFvIBBLBase
    dd 0xFEF60000   ; .IBBLBase   = .IBBBase + .IBBSize
    dd 0x1000       ; .IBBLSize    = PcdFlashFvIBBLSize = FLASH_REGION_FV_IBBL_SIZE in .fdf
    dd 0xFEF61000   ; .FITBase     = IBBLBase + IBBLSize
    dd 0xFEF61800   ; .TempRamBase >= FITBase + 0x100
    dd 0x9E800      ; .TempRamSize <= (CarBase + CarSize) - TempRamBase 
    dd 0            ; .HostToCse
    dd 0            ; .CseToHost
    dd 0            ; .ChunkIndex
    dd 0            ; .NumberOfChunks
...
%else
  ...
  ;
    cld
    mov     edi, [ADDR_OF(HOBStructure) + HobStruc.TempRamBase]
    mov     ecx, [ADDR_OF(HOBStructure) + HobStruc.TempRamSize]
    shr     ecx, 2
...
;    ShowPostCode 0x1000000
    ...
    cmp edx, 0
    je uCodeNotLoad
    mov ebx, 0x40000000   ;Bit 30 = uCodeStatus

uCodeNotLoad:
    or  ebx, 0x80000000   ;Bit 31 = Ready for RBP
    mov eax, dword [ebp + HobStruc.IBBSize]
    shl eax, 4
    and eax, 0x3FFFFFF0   ;Bits [27:14] = IBBSize
    add eax, ebx
...
;%ifdef BXTI_PF_ENABLE
    ...
    mov eax, [esi]       ; get the value that contians the FIT table size [N+1][23:0]
...
StartLoop:
    ...
    mov eax, [esi + 0x4] ; FIT entry type [N+1][54:48]
    and eax, 0x007f0000  ; mask off the FIT entry type
    shr eax, 0x10
    mov edx, 0x10
    cmp eax, edx         ; check for FIT entry type 0x10 = TXE Secure Boot
    jne ContLoop

    mov eax, [esi]       ; FIT entry sub-type [N+1][31:24]
    and eax, 0xff000000  ; mask off the FIT sub-entry
    shr eax, 0x18

    mov edx, 0x02
    cmp eax, edx         ; check for FIT sub-type 0x2 = TXE Measurement Hash
    je CopyTxeHash

    mov edx, 0x03
    cmp eax, edx         ; check for FIT sub-type 0x3 = Boot Policy
    je CopyBootGuard
...
CopyTxeHash:
    mov edi, esi
    mov eax, 0x1
    mov [ebp + HobStruc.TxeHashExist], eax

    mov eax, [esi - 0x8] ; get the address of TXE measurement hash
    mov esi, eax
    mov edx, 0
LoopHash:
    mov al, [esi + edx]
    mov [ebp + HobStruc.TxeHash + edx], al
    add edx, 0x1
    mov eax, 0x20        ; size of hash = 32 bytes
    cmp eax, edx
    jne LoopHash
...
CopyBootGuard:
    mov edi, esi
    mov eax, 0x1
    mov [ebp + HobStruc.BootPolicyExist], eax

    mov eax, [esi - 0x8] ; get the address of Boot Policy
    mov esi, eax
    mov ax, [esi]
    and ax, 0xc          ; bit 2: VB, bit 3: MB
    shr ax, 0x2
    mov [ebp + HobStruc.BootGuard], ax
    mov esi, edi
    jmp ContLoop
