...
#define USB_ATTRIBUTES_REMOTE_WAKEUP    BIT5
...
//[-start-180615-IB07400976-add]//
  if (NeedReEnumeration(RootHubController)) {
    DEBUG ((gUSBErrorLevel," Retry RootHubEnumeration !!\n"));
    RootHubEnumeration(0, RootHubController);
  }
//[-end-180615-IB07400976-add]//
  ...
  )
{
  ...
  if (CmosData & B_CMOS_S5_WAKE_ON_USB) {
    Status = UsbIo->UsbGetConfigDescriptor (UsbIo, &ConfigDescriptor);
    if ((!EFI_ERROR (Status)) && (ConfigDescriptor.Attributes & USB_ATTRIBUTES_REMOTE_WAKEUP)) {
      Status = mPrivate->UsbCore->UsbSetDeviceFeature (
        UsbIo,
        (UINT8)USB_DEV_SET_FEATURE_REQ_TYPE_D,
        (UINT16)USB_FEATURE_REMOTE_WAKEUP,
        0,
        &TxStatus
        );
    }
...
//[-start-170626-IB07400880-add]//
          ConfigRemoteWakeup (&NewController->UsbIo);
//[-end-170626-IB07400880-add]//
