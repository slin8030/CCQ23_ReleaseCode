...
//[-start-170505-IB07400864-add]//
  //
  // Disable Keyboard Interface before read data
  //
  KeyboardCommand (ConsoleIn, 0xAD);
//[-end-170505-IB07400864-add]//
...
//[-start-170428-IB07400864-modify]//
    if (Data != 0x00) {
      if (Data == 0xE0) {
        mE0Count++;
        if (mE0Count > 1) { // Multi E0 occur
          continue;
        }
      } else if (mE0Count) {
        if (mE0Count > 1) {
          PushScancodeBufTail (&ConsoleIn->ScancodeQueue, Data);
          PushScancodeBufTail (&ConsoleIn->ScancodeQueue, 0xE0);
          mE0Count--;
          continue;
        }
        mE0Count--;
...
//[-start-170505-IB07400864-add]//
  //
  // Restore Keyboard Interface after read data
  //
  KeyboardCommand (ConsoleIn, 0xAE);
//[-end-170505-IB07400864-add]//
  ...
  //
    ...
    for (Index = 0; ConvertKeyboardScanCodeToEfiKey[Index].ScanCode != TABLE_END; Index++) {
      if (ScanCode == ConvertKeyboardScanCodeToEfiKey[Index].ScanCode) {
        ...
        //
        if (ConsoleIn->LeftCtrl || ConsoleIn->RightCtrl) {
         if (KeyData.Key.UnicodeChar == '[' || KeyData.Key.UnicodeChar == '{') {
