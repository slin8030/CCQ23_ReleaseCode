...
EFI_PEI_RSC_HANDLER_CALLBACK mDdtStatusCodeReportWorkerBeforeShadow = NULL;
...
//[-start-160329-IB07400711-modify]//
      if (mDdtStatusCodeReportWorkerBeforeShadow == NULL) {
        Status = RscHandlerPpi->Register (DdtStatusCodeReportWorker);
        ASSERT_EFI_ERROR (Status);
        mDdtStatusCodeReportWorkerBeforeShadow = DdtStatusCodeReportWorker;
      } else {
        Status = RscHandlerPpi->Unregister (mDdtStatusCodeReportWorkerBeforeShadow);  
        Status = RscHandlerPpi->Register (DdtStatusCodeReportWorker);
        ASSERT_EFI_ERROR (Status);      
      }
