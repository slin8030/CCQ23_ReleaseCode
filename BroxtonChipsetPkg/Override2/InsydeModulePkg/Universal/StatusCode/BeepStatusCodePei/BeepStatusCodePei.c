...
EFI_PEI_RSC_HANDLER_CALLBACK mBeepStatusCodeReportWorkerBeforeShadow = NULL;
  ...
  )
{
  ...
  if (!EFI_ERROR(Status)) {
    ...
    //
//[-start-160929-IB05400712-modify]//
    if (mBeepStatusCodeReportWorkerBeforeShadow == NULL) {
      Status = RscHandlerPpi->Register (PeiBeepStatusCode);
      ASSERT_EFI_ERROR (Status);
      mBeepStatusCodeReportWorkerBeforeShadow = PeiBeepStatusCode;
    } else {
      Status = RscHandlerPpi->Unregister (mBeepStatusCodeReportWorkerBeforeShadow);  
      Status = RscHandlerPpi->Register (PeiBeepStatusCode);
      ASSERT_EFI_ERROR (Status);      
    }
//[-end-160929-IB05400712-modify]//
...
/**
  ...
  @param  BeepTypeList  The list of beep type (comes from PCD or OEM services).
...
//[-start-160929-IB05400712-modify]//
      BeepTypeIndex = GetBeepType (BeepTypeList, BeepEntry->BeepTypeId);
//[-end-160929-IB05400712-modify]//
