...
//[-start-170208-IB07400839-add]//
    //
    // Check this BlockIoPpi already start or Not.
    //
    AlreadyStart = FALSE;
    if (PrivateData->CapsuleCount != 0) {
      for (Index = 0; Index < PrivateData->CapsuleCount; Index++) {
        if (PrivateData->CapsuleData[Index].BlockIo == BlockIoPpi) {
          AlreadyStart = TRUE;
          break;
        }
      }
...
//[-start-170208-IB07400839-add]//
    DEBUG ((EFI_D_INFO, "NumberBlockDevices = %x\n", NumberBlockDevices));
//[-end-170208-IB07400839-add]//
...
//[-start-170208-IB07400839-add]//
      DEBUG ((EFI_D_INFO, "Media.MediaPresent = %x\n", Media.MediaPresent));
      DEBUG ((EFI_D_INFO, "Media.DeviceType = %x\n", Media.DeviceType));
//[-end-170208-IB07400839-add]//
      if (EFI_ERROR (Status) ||
          !Media.MediaPresent ||
...
//[-start-170208-IB07400839-add]//
  if (AsciiStrLen ((CHAR8 *)FileName) > 8) {
    UnicodeStrToAsciiStr (PcdGetPtr(PcdPeiRecoveryShortFileName), (CHAR8 *)FileName);
  }
  DEBUG ((EFI_D_INFO, "RetrieveCapsuleFileFromRoot: RecoveryFileName = %a\n", FileName));
//[-end-170208-IB07400839-add]//
  SetMem (Buffer, BufferSize, 0);
...
//[-start-170208-IB07400839-add]//
  ...
  DEBUG ((EFI_D_INFO, "VolumeLabelMatched: RecoveryFileName = %a\n", FileName));
//[-end-170208-IB07400839-add]//
