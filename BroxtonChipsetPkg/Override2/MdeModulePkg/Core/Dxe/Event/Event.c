...
#ifdef INSYDE_DEBUGGER
    {
      if (CompareGuid (&Event->EventGroup, &gEfiEventExitBootServicesGuid)) { // Exit boot services event debug
        POST_CODE (0xDA00);
      } else if (CompareGuid (&Event->EventGroup, &gEfiEventReadyToBootGuid)) { // Ready to boot event debug
        POST_CODE (0xDA01);
      } 
...
{
//[-start-160828-IB07400775-remove]//
//  //
