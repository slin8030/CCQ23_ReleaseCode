        ...
        Method (_ON, 0, Serialized)
        {
            If (LEqual (MSD3, 0x03))
            {
                ...
                If (LEqual (DGPU_SCOPE.SGGP, 0x01))
                {
                    ...
                    Store (Zero, DGPU_SCOPE.MLTF) // Disable HDA multifunction device
                }
                Store (1, _STA)
                Store (0, MSD3)
            }
...
Scope (DGPU_SCOPE)
{
    ...
    OperationRegion (VGAR, SystemMemory, DGPU_SCOPE.DDPA, 0x100)
    ...
    OperationRegion (DGPU, SystemMemory, DGPU_SCOPE.DDPA, 0x50)
    Field (DGPU, DWordAcc, NoLock, Preserve)
    {
        DVVD, 16,
        Offset (0x40), // SSID SVID
        SSSV, 32,
    }
 ...
 //
  ...
  Method(HGPO, 4, Serialized)
  {
    ...
    If (LEqual(Arg3, 1))
    {
      ...
      Store(Or(TEMP,0x00000002), TEMP)
 

    }
    ElseIf (LEqual(Arg3, 0))
    {
    
      Not(0x00000002, Local0)
