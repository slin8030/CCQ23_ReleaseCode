        ...
        If(Lequal(Arg1, 1)){
          ...
          Method (SPPS,2,Serialized) {
            ...
            Field(XDBW, WordAcc, NoLock, Preserve)
            {
              ...
              Offset(0x10f80c),  //APBFC_U3PMU_CFG1
	               , 2,
	          CSEB , 1,      //Configuration Selection Enable
                   , 1,
 	          SWID , 2,      //SW ID enable
              	   , 9,
              CHVB , 1,		 //Configuration by xDCI
              Offset(0x10f810),
                  ,  8,
...
//#if BXTI_PF_ENABLE
              If (LEqual(IOTP(),0x01)) { // IOTG Board IDs
                If(LEqual(\OSSL,3)) // Linux
                {
                  //Deassert the core soft reset
                  Store(0, PURC)
                }  
              }
//#endif
...
//#if BXTI_PF_ENABLE
              If (LEqual(IOTP(),0x01)) { // IOTG Board IDs
                ...
                If(LEqual(\OSSL,3))
                {
                  //0x10f818 Bit 3 - Assert the core soft reset
                  Store(1, PURC)
                }
              }
//#endif
