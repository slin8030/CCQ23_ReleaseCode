  ...
  )
{
  ...
  for (Index = 0;
       Index < mAlwaysCseStoredVariablesLength;
       Index++) {
    if (CompareGuid (VendorGuid, mAlwaysCseStoredVariables[Index].VendorGuid) &&
        (StrCmp (VariableName, mAlwaysCseStoredVariables[Index].VariableName) == 0)) {
      return TRUE;
    }
  }
  return FALSE;
...
{
  if ((PcdGetBool (PcdHostManagedVariables) == TRUE) &&
      !IsCseVariable (VariableName, VendorGuid)      &&
      BootMediaIsSpi ()) {
