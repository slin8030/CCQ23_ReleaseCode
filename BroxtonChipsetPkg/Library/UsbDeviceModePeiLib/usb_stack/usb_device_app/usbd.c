        ...
        if (gDrvObj.mmioBar != 0) {
            ...
            if (udciInit(gDrvObj.mmioBar, &gDrvObj.pDciDrvObj) == true) {
                ...
                if (udciRegisterCallbacks(gDrvObj.pDciDrvObj) == true) {
                    ...
                    DEBUG ((DEBUG_INFO, "\nusbdInitDCI() - Controller initialization complete\n"));
                } else {
...
bool usbdRun(uint32_t timeoutMs) {
    ...
    if ((gDrvObj.dciInitialized == true)) {
        ...
        while (1) {
            ...
            /* check for timeout */
            if (timeoutMs == 0) {
                /* If we are not yet connected (still at INIT), check for timeout */
                if (gDrvObj.state == USBD_STATE_INIT) {
                }
            }
                delayMicro(10);
//                timeoutMs--;
...
//                  break;
//                }  
                //DEBUG ((DEBUG_INFO, "\r%x", timeoutMs));
        }
    }
