  ...
  )
{
  ...
  Status = SaveLockBox (
             ...
             mS3BootScriptTablePtr->TableMemoryPageNumber * EFI_PAGE_SIZE
             );
...
EFI_STATUS
EFIAPI
S3BootScriptLibDestructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  if (mEnterRuntimeEvent != NULL) {
    gBS->CloseEvent (mEnterRuntimeEvent);
    PcdSet64 (PcdS3BootScriptTablePrivateDataPtr, 0);
    if (mS3BootScriptTablePtr != NULL) {
      gBS->FreePages ((EFI_PHYSICAL_ADDRESS) (UINTN) mS3BootScriptTablePtr, EFI_SIZE_TO_PAGES(sizeof(SCRIPT_TABLE_PRIVATE_DATA)));
    }
  }
  ...
  )
{
  ...
  Status = UpdateLockBox (
             ...
             ((EFI_BOOT_SCRIPT_COMMON_HEADER*)Script)->Length + sizeof (EFI_BOOT_SCRIPT_TERMINATE)
             );
  ASSERT_EFI_ERROR (Status);
}
