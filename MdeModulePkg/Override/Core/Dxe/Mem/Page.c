...
/**
  ...
  @param  StartAddress           Pointer to start address

  @retval EFI_SUCCESS            Find free page successfully.
  @retval EFI_NOT_FOUND          Cannot find free page.

**/
STATIC
EFI_STATUS
CoreFindFreePagesIEx (
  IN   UINT64           MaxAddress,
  IN   UINT64           MinAddress,
  IN   UINT64           NumberOfPages,
  IN   EFI_MEMORY_TYPE  NewType,
  IN   UINTN            Alignment,
  OUT  UINT64           *StartAddress
  )
{
  UINT64          NumberOfBytes;
  UINT64          Target;
  ...
  )
{
  ...
  if (Type != AllocateAddress) {
    ...
    MaxAddress = Start + NumberOfPages * EFI_PAGE_SIZE - 1;
    Status = CoreFindFreePagesIEx (MaxAddress, Start, NumberOfPages, MemoryType, EFI_PAGE_SIZE, &Start);
    if (Status != EFI_SUCCESS) {
      goto Done;
    }
  }
