...
SYS_PASSWORD_PRIVATE                    mPasswordInstance;
SYS_PASSWORD_SETUP_INFO_DATA            mSysPswdSetupInfoData;

EFI_STATUS
EFIAPI
EfiGetSystemConfigurationTable2 (
  ...
  )
{
  ...
  *Table = NULL;
  for (Index = 0; Index < SystemTable->NumberOfTableEntries; Index++) {
    if (CompareGuid (TableGuid, &(SystemTable->SmmConfigurationTable[Index].VendorGuid))) {
      *Table = SystemTable->SmmConfigurationTable[Index].VendorTable;
      return EFI_SUCCESS;
    }
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    if (!IsUserPassword) {
      Status = EfiGetSystemConfigurationTable2 (&gEfiHobListGuid, (VOID *) &HobList);
      if (Status == EFI_SUCCESS) {
  ...
  )
{
  ...
  PasswordInstance = &mPasswordInstance;
...
//    return Status;
//  }
//[-end-140410-00000-remove]//

  //
  //  Initialize the setup info data
  //
  SysPswdSetupInfoData = NULL;
  SysPswdSetupInfoData = &mSysPswdSetupInfoData;
