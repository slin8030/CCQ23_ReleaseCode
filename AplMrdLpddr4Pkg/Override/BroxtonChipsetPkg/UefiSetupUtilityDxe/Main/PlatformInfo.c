...
//[-start-160418-IB07400718-modify]//
  ...
  KscFwRevL = 0;
#else
//[-start-171221-IB07400935-add]//
  UINT8          KscFwMainVersion;
  UINT8          KscFwSubVersion;
  UINT8          KscFwTestVersion;

  KscFwMainVersion = 0;
  KscFwSubVersion = 0;
  KscFwTestVersion = 0;
//[-end-171221-IB07400935-add]//
#endif
#endif
...
//[-start-160726-IB07400762-add]//
  ...
  UnicodeSPrint (Buffer, sizeof (Buffer), L"%d.%d.%d", (SrcVersion >> 24) & 0xFF, (SrcVersion >> 16) & 0xFF, (SrcVersion >> 8) & 0xFF);
  NewStringId = HiiSetString (MainHiiHandle, (EFI_STRING_ID)0, Buffer, NULL);
  NewStringToHandle (
    AdvanceHiiHandle,
...
//[-start-171221-IB07400935-modify]//
  ...
  if (IoRead8(0x62) != 0xFF) { // EC exist
    ...
    InitializeKscLib();
    Status = SendKscCommand(0x80);    // READ command
    Status = SendKscData(0x00);       // EC FW main version
    Status = ReceiveKscData(&KscFwMainVersion);
    
    Status = SendKscCommand(0x80);    // READ command
    Status = SendKscData(0x01);       // EC FW sub version
    Status = ReceiveKscData(&KscFwSubVersion);
    
    Status = SendKscCommand(0x80);    // READ command
    Status = SendKscData(0x02);       // EC FW test version
    Status = ReceiveKscData(&KscFwTestVersion);

    if(!EFI_ERROR(Status)) {
      //Status = ReceiveKscData(&KscFwRevH);
      //Status = ReceiveKscData(&KscFwRevL);
      if(!EFI_ERROR(Status)){
        UnicodeSPrint (Buffer, sizeof (Buffer), L"%02X.%02X.%02X", KscFwMainVersion, KscFwSubVersion, KscFwTestVersion);
        NewStringId = HiiSetString (MainHiiHandle, (EFI_STRING_ID)0, Buffer, NULL);
        NewStringToHandle (
          AdvanceHiiHandle,
          STRING_TOKEN (STR_KSC_FW_STRING),
