...
#include <Library/EventLogSmmLib.h>
...
EVENT_LOG_PROTOCOL  *mEventLogSmm=NULL;
  ...
  )
{
  ...
  Status = gSmst->SmmLocateProtocol (&gH2OSmmEventLogProtocolGuid, NULL,(VOID **)&mEventLogSmm);
//  if (EFI_ERROR (Status)) {
//    return Status;
//  }
  ...
  )
{
  ...
  Status = mEventLogSmm->LogEvent(EventStorageType, EventID, ErrorData, ErrorDataSize);
  ...
  )
{
  ...
  Status = mEventLogSmm->GetEvent(StorageString, DataIndex, Data);
  ...
  )
{
  ...
  Status = mEventLogSmm->GetEventCount(StorageString, EventLogCount);
  ...
  )
{
  ...
  Status = mEventLogSmm->ClearStorage(StorageString);
  ...
  )
{
  ...
  Status = mEventLogSmm->RefreshDatabase(StorageString, EventLogCount);
  ...
  )
{
  ...
  Status = mEventLogSmm->GetStorageName((EVENT_STORAGE_INFO**)StorageName, StorageNum);
